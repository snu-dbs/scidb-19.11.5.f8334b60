...
static Result execute_testcase (struct InfoForExecutor &ie)
{
	...
	boost::posix_time::ptime the_epoch(boost::gregorian::date(1970,1,1));
	...
	try
	{
		...
		sTime = (boost::posix_time::microsec_clock::universal_time()-the_epoch).total_milliseconds();
		...
		eTime = (boost::posix_time::microsec_clock::universal_time()-the_epoch).total_milliseconds();
		delete caseexecutor;
		caseexecutor = 0;
		...
		boost::posix_time::ptime t_ptime=  boost::posix_time::microsec_clock::universal_time();
