...
void XMLArchive :: save (const struct IndividualTestInfo &iti)
{
	...
	string TestStartTime = iso8601_localtime (&startTime);
	string TestEndTime = iso8601_localtime (&endTime);
	...
	string TestTotalExeTime = boost::lexical_cast<std::string> (
            (double(time_t(iti.testEi.eTime) - time_t(iti.testEi.sTime)))/1000);
	putCHAR (XML_CHAR_TAB);
