...
echo "Creating array:"
${IQUERY_CMD} -aq "create array dummy_01_A_$script_${MY_PID} ${FLAT_SCHEMA}"
...
then
    ${IQUERY_CMD} -ocsv:l -naq "load(dummy_01_A_$script_${MY_PID},'${MY_PID}_input',-1,'(int64,int64,${ATTR_BIN_TYPES})')"
else
    ${IQUERY_CMD} -ocsv:l -naq "load(dummy_01_A_$script_${MY_PID},'${MY_PID}_input',-1,'${DATA_FORMAT}')"
fi

echo "Count d_0 attributes:"
${IQUERY_CMD} -ocsv:l -aq "aggregate(dummy_01_A_$script_${MY_PID},count(d_0))"

echo "Count d_1 attributes:"
${IQUERY_CMD} -ocsv:l -aq "aggregate(dummy_01_A_$script_${MY_PID},count(d_1))"

echo "Count a_1 attributes:"
${IQUERY_CMD} -ocsv:l -aq "aggregate(dummy_01_A_$script_${MY_PID},count(a_1))"
