...
then
    error_text=$(${IQUERY_CMD} -ocsv:l -naq "load(dummy_01_A_$script_${MY_PID},'${MY_PID}_input',-1,'(int64,int64,${ATTR_BIN_TYPES})')" 2>&1 | grep "Error id: scidb::SCIDB_SE_IMPORT_ERROR::SCIDB_LE_FILE_IMPORT_FAILED")
else
    error_text=$(${IQUERY_CMD} -ocsv:l -naq "load(dummy_01_A_$script_${MY_PID},'${MY_PID}_input',-1,'${DATA_FORMAT}'" 2>&1 | grep "Error id: scidb::SCIDB_SE_IMPORT_ERROR::SCIDB_LE_FILE_IMPORT_FAILED")
fi
