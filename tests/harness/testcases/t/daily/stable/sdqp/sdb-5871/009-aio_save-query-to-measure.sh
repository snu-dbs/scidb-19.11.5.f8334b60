...
time iquery -aq "
        aio_save(apply(
          cross_between(
           project(public.MEASUREMENT_KINECT, SpineShoulder_X, SpineShoulder_Floor_Y, SpineShoulder_Z),
           project(
            apply(
             cross_join(
              project(
               filter(public.MEASUREMENT, sub_device_site='Variable' and measurement_type='KINECT'),
               sub_device_site
              ) as MEASUREMENT,
             ...
             , extra_end_0, 4611686018427387904
            ),
          ...
          millis, millis
         ), '/tmp/011-query-to-measure-aio-save-data')"
