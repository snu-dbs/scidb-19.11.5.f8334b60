...
time iquery -aq "
         consume(apply(
          cross_between(
           public.MEASUREMENT_QUATERNION,
           project(
            apply(
             cross_join(
              project(
               filter(public.MEASUREMENT, sub_device_site='Lumbar' and measurement_type='QUATERNION'),
               sub_device_site
              ) as MEASUREMENT,

              project(
               cross_join(
                filter(public.EVENT, event_short_label='GaitWalk10m' and subject_visit_id = 1) as EVENT,
