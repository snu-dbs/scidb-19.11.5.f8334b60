          ...
          cross_between(
           ...
           project(
            apply(
             cross_join(
              project(
               filter(public.MEASUREMENT, sub_device_site='Right Wrist' and measurement_type='ANGULAR_VELOCITY'),
               sub_device_site
              ) as MEASUREMENT,

              project(
               cross_join(
                filter(public.EVENT, event_short_label='RestTremor10S' and subject_visit_id = 1) as EVENT,
