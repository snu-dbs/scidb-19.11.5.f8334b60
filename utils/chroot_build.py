...
class UbuntuChroot():
    ...
    def __init__(self, release, arch, temp_dir):
        ...
        if self.release == "trusty":
            self.scidb_3rdparty_mirror='deb https://downloads.paradigm4.com/ 3rdparty/19.11/trusty/'
            self.compiler_mirror='deb http://ppa.launchpad.net/ubuntu-toolchain-r/test/ubuntu trusty main'
            self.mkl='deb https://downloads.paradigm4.com/ mkl/'
            self.mirror = '|'.join([(self.ubuntu_mirror % release), self.scidb_3rdparty_mirror, self.compiler_mirror, self.mkl])
        # xenial is Ubuntu 16.04 release
        elif self.release == "xenial":
            self.scidb_3rdparty_mirror='deb https://downloads.paradigm4.com/ 3rdparty/19.11/xenial/'
            # self.compiler_mirror='deb http://ppa.launchpad.net/ubuntu-toolchain-r/test/ubuntu xenial main'
            self.mkl = 'deb https://apt.repos.intel.com/mkl all main'
            self.mirror = '|'.join([(self.ubuntu_mirror % release), self.scidb_3rdparty_mirror, self.mkl])
        else:
